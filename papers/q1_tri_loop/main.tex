...
of energy duration is consistent with industry sizing for primary
frequency support. Active and reactive power loops are decoupled in
...
distribution feeder serving the mixed light-industrial and
commercial load typical of Indonesian secondary cities, where the
Java--Bali bulk system provides only modest residual rotational
support during daytime PV peak. Three site characteristics shape the
operating envelope under which the controller must perform: first,
frequent equatorial cumulus events produce sub-minute irradiance
ramps that load the BESS share $\beta$ in seconds-scale bursts and,
in the absence of supervisory coordination, alias with the dead-band
of the adaptive law; second, the daytime short-circuit ratio at the
point of connection rarely exceeds 2, placing the plant near the
...
third, the diurnal load--temperature coupling characteristic of an
Indonesian tropical grid produces a multi-scale forcing---fast cloud
transients superposed on a slow daily envelope---that conventional
gain-scheduled VIC schemes do not anticipate. These conditions
translate directly into the canonical scenarios of
Section~\ref{sec:setup}---S2 (cloud passing), S5 (weak grid), and
S6 (24-h Madiun day)---and the limit-cycle phenomenon identified in
this paper emerges precisely where they intersect: not as a
theoretical corner case but as a quantitative design constraint with
measurable consequences in BESS cycling and battery-degradation
cost.
