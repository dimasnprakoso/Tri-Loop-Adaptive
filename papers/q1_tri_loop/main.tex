...
\author{Dimas~Nur~Prakoso,
        ...
        and~Basuki~Winarno%
\thanks{Manuscript submitted on May~29, 2026. This work was supported by the Direktorat Penelitian dan Pengabdian kepada Masyarakat (DPRM) Institut Teknologi Sepuluh Nopember under the PKAPT~2026 research grant. The authors declare no conflict of interest related to this work.}%
...
\thanks{Source code, raw simulation data, and reproducibility manifest:\\\url{https://github.com/dimasnprakoso/Tri-Loop-Adaptive}.}}
...
the FFT peak amplitude by $\sim$3 orders of magnitude while satisfying
...
including an empirical equivalence between the fuzzy and tanh-smooth
schemes that, to our knowledge, is not previously documented in this
PV-BESS-VIC context.
...
The plant under study is the $125\,$kWp grid-tied rooftop
installation in Winongo, Kota Madiun, East Java, Indonesia
(Fig.~\ref{fig:plant_real}; $7.613^\circ$S, $111.512^\circ$E),
comprising $228$ Chint Astro~5 Semi $550\,$Wp PV modules, a
50\,kWh / 60\,kW lithium-ion BESS, and a single three-phase
voltage-source inverter behind an LCL filter. The PV array is represented in the
simulation by a reduced-order single-diode aggregate matched to the
plant MPP at STC; this installation also drives the Madiun case study
in Section~\ref{sec:results-madiun}. The $125\,$kW rating places the
plant within IEEE~Std~1547-2018 Category~I (devices $\leq$1\,MVA),
and the $P_\mathrm{bess}/P_\mathrm{pv}=0.48$ ratio with $\sim$50\,min
of energy duration is consistent with sizing guidelines reported for
BESS-supported primary frequency response
\cite{trovato2024nadir,paturet2023fast}. Active and reactive power loops are decoupled in
the synchronous reference frame; we focus on the active-power loop
because the studied phenomena (RoCoF, frequency nadir, limit cycle)
are dominated by it
...
\centering
\includegraphics[width=\linewidth,height=3.3cm,keepaspectratio]{fig_plant_install}
\caption{Module installation}
\label{fig:plant_install}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.50\columnwidth}
\centering
\includegraphics[width=\linewidth,height=3.3cm,keepaspectratio]{fig_plant_overview}
\caption{Completed rooftop array}
\label{fig:plant_overview}
\end{subfigure}
\caption{The 125\,kWp PV plant in Madiun, Indonesia, that the model
parameters in this paper mirror.}
\label{fig:plant_real}
\end{figure}

Fig.~\ref{fig:plant_install} captures the array during commissioning
and Fig.~\ref{fig:plant_overview} the completed system under tropical
clear-sky irradiance. The plant sits at the end of a medium-voltage
distribution feeder typical of Indonesian secondary cities, where
three site characteristics shape the operating envelope: frequent
equatorial cumulus events produce sub-minute irradiance ramps that
load the BESS share $\beta$ in seconds-scale bursts; the daytime
short-circuit ratio rarely exceeds 2, placing the plant near the
marginal-stability boundary of grid-following inverter PLLs
\cite{eckel2024classification,hatziargyriou2021classification}; and
the diurnal load--temperature coupling produces a multi-scale
forcing that conventional gain-scheduled VIC does not anticipate.
These conditions translate directly into the canonical scenarios of
Section~\ref{sec:setup}---S2 (cloud passing), S5 (weak grid), and
S6 (24-h Madiun day)---and the limit-cycle phenomenon identified
in this paper emerges precisely where they intersect.
...
match the converter's transient overload rating. Design parameters
take the values $H_\mathrm{min}=\SI{1}{\second}$,
$H_\mathrm{max}=\SI{5}{\second}$, $D_\mathrm{min}=12$,
$D_\mathrm{max}=50$, $\gamma_J=\delta_D=1$, $k_\alpha=4$,
$k_\alpha^{\dot\omega}=1$, $\beta_\mathrm{max}=0.7$, and
$\bar{\mathrm{SOC}}=(SOC_\mathrm{min}+SOC_\mathrm{max})/2=0.55$;
full parameter set in
\texttt{julia/src/models/params.jl} of the released code.
...
\begin{table}[t]
\caption{The five control schemes evaluated. C1--C4 share identical
$[H_\mathrm{min},H_\mathrm{max}]$, $[D_\mathrm{min},D_\mathrm{max}]$,
and BESS rating, so performance differences reflect the gain schedule
and coordination logic only.}
\label{tab:schemes}
\centering
...
S5 & low-inertia/low-damping weak-grid proxy ($H_\mathrm{sys}=D_\mathrm{sys}=1$) + S1 & 1.0 & 8\,s \\
...
\input{tables/comparative_table}
...
the ENTSO-E $1$\,Hz/s envelope; pass rate under Cat.~II $2$\,Hz/s is
$\geq$~R$_{\mathrm{1Hz/s}}$ by monotonicity (also $100\%$ for C4).}
\label{tab:mc-pass}
\centering
...
$\dot\omega_\mathrm{ref}\!\in\![0.5,3]$\,Hz/s,
$k_\alpha\!\in\![2,10]$,
$\beta_\mathrm{max}\!\in\![0.1,1.0]$,
$H_\mathrm{max}\!\in\![1,8]$\,s
(viridis: low$\to$high). All loci in the LHP.}
...
A frozen-time off-equilibrium linearization (Fig.~\ref{fig:eigen_offeq},
Section~\ref{sec:discussion-lc}) addresses this restriction by
evaluating the Jacobian at sustained-fault operating points where the
adaptive gains have departed $(H_\mathrm{min},D_\mathrm{min})$.
...
the measurement-filter pole at $f_\mathrm{meas}=1/(2\pi T_\mathrm{meas})=7.96$\,Hz.
...
converter dead-time, and represents the PV array as a single-diode
MPP-fit aggregate (module-level mismatch and partial shading
abstracted). The averaged-model code is verified bit-for-bit
...
$V_\mathrm{DC}$, no switching) against a Simscape EMT model, and
hardware-in-the-loop emulation on a real-time target, are scoped as
revision work following \cite{wang2022describing,kim2025selfexcited}.
...
\url{https://github.com/dimasnprakoso/Tri-Loop-Adaptive}; the Julia
